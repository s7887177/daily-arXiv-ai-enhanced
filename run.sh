...
# STOPGAP wiring (run.* are temporary, to be refined): the crawler picks the
# output filename from the RSS feed date (pubDate) and prints that path to
# stdout. write_jsonl truncates, so no pre-delete is needed.
out=$(uv run python -m daily_arxiv_rss.crawl)
today=$(basename "$out" .jsonl)   # feed pubDate; all later steps key off ${today}
...
# 第二步：检查去重 / Step 2: Check duplicates (our dedup, RSS-date aligned)
echo "步骤2：执行去重检查... / Step 2: Performing dedup..."
uv run python -m daily_arxiv_rss.dedup --data "$out"
dedup_exit_code=$?

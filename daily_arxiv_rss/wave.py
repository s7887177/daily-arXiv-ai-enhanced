...
newest arXiv id first, and writes one ``agent-NN.jsonl`` per subagent.
...
CLI:  uv run python -m daily_arxiv_rss.wave --date 2026-05-19 --wave 100 --per 10
"""
...
def build_waves(date: str, wave_size: int, per_agent: int,
                out_dir: str = "/tmp/digest-wave", repo_root: str = ".",
                min_pdf_bytes: int = 1000, min_age_s: float = 8.0) -> dict:
    root = pathlib.Path(repo_root)
    prune_empty_pdfs(str(root / "pdfs"))

    recs: dict[str, dict] = {}
    for line in open(root / f"data/{date}.jsonl", encoding="utf-8"):
        if line.strip():
            r = json.loads(line)
            recs[r["id"]] = r
    ...
    for p in glob.glob(str(root / "pdfs/*.pdf")):
        ...
        if (st.st_size > min_pdf_bytes and now - st.st_mtime > min_age_s
            ...
            stable.append(sid)
    stable.sort(reverse=True)                      # newest first
...
def main(argv=None) -> int:
    ...
    s = build_waves(a.date, a.wave, a.per, a.out, a.repo_root)
